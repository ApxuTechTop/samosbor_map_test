namespace $ {
	...
	export class $apxu_samosbor_map_gigacluster extends $giper_baza_dict.with( {
		...
		delete_block( ref: any ) {
			...
			const block = $giper_baza_glob.Node( ref, $apxu_samosbor_map_block_data )
			block.transitions()?.forEach( ( transition ) => {
				...
				const connected_block = $giper_baza_glob.Node( connected_block_ref, $apxu_samosbor_map_block_data )
		...
		transition( block: $apxu_samosbor_map_block_data, floor: number, position: TransitionPosition ) {
			return block?.transitions()?.find( ( trans ) => {
				const check = ( port: TransitionPort | undefined | null ) => {
					...
					if( !block_ref ) return
					if( $giper_baza_glob.Node( block_ref, $apxu_samosbor_map_block_data ) === block && Number( port.Floor( null )?.val() ) === floor && port.Position( null )?.val() === position ) {
						return true
					}
				}
