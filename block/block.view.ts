namespace $.$$ {
	...
	export class $apxu_samosbor_map_block extends $.$apxu_samosbor_map_block {
		...
		change_connection( position: TransitionPosition ) {
			...
			const first_block = $giper_baza_glob.Node( first_port.block_ref, $apxu_samosbor_map_block_data )
			...
			if( transition ) {
				...
				// соединить блоки
				const another_block = $giper_baza_glob.Node( first_port.block_ref, $apxu_samosbor_map_block_data )
